...
with app.app_context():
    ...
    for table in tables:
        try:
            ...
            status = "✓ OK" if count >= 10 else "⚠️  LOW" if count > 0 else "✗ EMPTY"
            print(f"  {status:8s} {table:20s}: {count:3d} rows")
    ...
    for row in faculty_data:
        print(f"  ID: {row[0]:10s} | Name: {row[1]:15s} | Email: {row[2]:30s} | Dept: {row[3]} | Password: 123456")
    ...
    for row in student_data:
        print(f"  ID: {row[0]:12s} | Name: {row[1]:12s} | Email: {row[2]:35s} | Dept: {row[3]:3s} | Sem: {row[4]} | Password: 123456")
